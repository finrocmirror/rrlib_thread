...
public:
   ...
   */
  inline static tThreadId CurrentThreadId()
  {
   ...
   *
   * The thread id is unique for quite a long time (until 2^32 threads
   * have been spawned. After that, it is guaranteed that all existing threads
   * have different ids. Ids repeat less than every 2^31 threads)
   * A thread id is never zero
   */
  inline tThreadId GetId() const
  {
    return id;
  }
   ...
   * (values as for Linux' niceness: -20 Highest priority, 0 default, 19 Lowest)
   * Can only be called for the current thread!
   */
